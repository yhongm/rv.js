...
var YrvElement = function () {
    ...
    function YrvElement(tag, props, children, belong, componentUniqueTag, uniqueTag, isComponent, renderCallback) {
        ...
        if (_yrvUtil2.default.isHtmlTag(this.tag) && !this.key) {
    ...
    */
    ...
    _createClass(YrvElement, [{
        ...
        value: function _handleSlotDom(node, slotDom, slotPosition) {
            ...
            var positionIndex = -1;
            if (position === "default") {
                positionIndex = _yrvUtil2.default.toArray(node.childNodes).findIndex(function (childNode) {
        ...
        value: function _innerHandlerProps(el, props) {
            ...
            var _loop = function _loop(propName) {
                if (!_yrvUtil2.default.isRvJsProp(propName)) {
                    if (_yrvUtil2.default.isRvEvent(propName)) {
                        evantName = propName.slice(3);

                        if (evantName == "watch") {
                            //this prop use to watch element value change in real time and auto to modify data
                            if (el instanceof HTMLInputElement) {
                                _yrvUtil2.default.addElementEventListener(el, "input", function (e) {
                                    _yrvUtil2.default.createAndSendSimpleRvEvent(_yrvUtil2.default.generateHashMNameByMName(_this4.belong + "_" + _this4.componentUniqueTag + "_" + props[propName] + "change"), el.value);
                                });
                            } else {
                                console.log("RV warning:the rv-watch only use in input label");
                            }
                        } else {
                            _yrvUtil2.default.addElementEventListener(el, evantName, function (e) {
                                Object.defineProperty(e, "element", {
                                    value: el
                                });
                                _yrvUtil2.default.createAndSendSimpleRvEvent(_yrvUtil2.default.generateHashMNameByMName(_this4.belong + "_" + _this4.componentUniqueTag + "_" + props[propName]), e);
                            });
                        }
                    } else {
                        if (props[propName] !== undefined) {
                            _yrvUtil2.default.setAttr(el, propName, props[propName]);
                        }
                    }
                }
            };
